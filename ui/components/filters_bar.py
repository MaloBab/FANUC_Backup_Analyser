...
class FiltersBar(tk.Frame):
    ...
    def _build(self) -> None:
        ...
        self._filter_var.trace_add("write", lambda *_: self._emit())

        entry_frame = tk.Frame(self, bg=PALETTE["bg_input"],
        ...
        entry.pack(side="left", padx=(6, 0), pady=3)
        ...
        self._filter_var.trace_add("write", self._on_text_changed)
        ...
        for value, label, accent in (
            ...
            self._pill_btns[value] = pill  # type: ignore[assignment]

        self._select_scope("all", emit=False)

        # ── Compteur (côté droit, injecté depuis MainPanel) ───────────
    ...
    def _select_scope(self, value: str, emit: bool = True) -> None:
        ...
        for v, pill in self._pill_btns.items():
            pill.set_active(v == value)  # type: ignore[attr-defined]
        if emit:
            self._emit()
...
class _ScopePill(tk.Frame):
    ...
    def set_active(self, active: bool) -> None:
        ...
        if active:
            ...
            self.config(bg=PALETTE["bg_input"],
